...
# (SRM_AND_SBI_MONOMER_DIMER_ALP_DETECTOR_HPC_Generate_Controller.sh): DRYRUN=1 prints, DRYRUN=0
...
#     bash SRM_AND_SBI_MONOMER_DIMER_ALP_DETECTOR_HPC_Submit.sh <stage> [KEY=VALUE ...]
#
...
#                   (a DLI-only pass over CONDITION's trajectory tier, which the biology
#                   dispatcher's RDS-only simulation with the same CONDITION generates
#                   (`simulation SIM_STAGE=rds CONDITION=<FAB|INLB>`; one tier per condition,
#                   shared by both workflows) -- the detector has no RDS stage; CONDITION =
#                   FAB|INLB, the labeling law, is always required)
...
# Fail loud if the result is not a real srm-and-sbi-monomer-dimer-alp checkout.
REPO="${REPO:-$(cd "$HERE/../.." && pwd)}"
if [ ! -f "$REPO/pyproject.toml" ] || [ ! -d "$REPO/srm_and_sbi_monomer_dimer_alp" ]; then
    echo "FATAL: '$REPO' is not a srm-and-sbi-monomer-dimer-alp repo root (needs pyproject.toml + srm_and_sbi_monomer_dimer_alp/)." >&2
    exit 1
fi
...
# are condition-specific -- the Simulation stage is a DLI pass over the condition's trajectory tier
# (one per condition, generated by the biology dispatcher's RDS-only simulation with the same
# CONDITION) under the condition's labeling law (the detector has no RDS stage) -- so the token is
# always required and forwarded. It enters the job name right after the qualified alias:
...
case "$STAGE" in
  simulation)
    SUBMIT_SCRIPT="$REPO/Script_Bank/HPC/SRM_AND_SBI_MONOMER_DIMER_ALP_DETECTOR_HPC_Simulation.sh"
