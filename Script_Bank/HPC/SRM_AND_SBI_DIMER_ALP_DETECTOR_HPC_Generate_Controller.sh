...
#         bash SRM_AND_SBI_DIMER_ALP_DETECTOR_HPC_Generate_Controller.sh
#     LIVE:
#         DRYRUN=0 bash SRM_AND_SBI_DIMER_ALP_DETECTOR_HPC_Generate_Controller.sh 2>&1 | tee ~/detector_gen_controller.log
#
...
#         --job-name=SRM_AND_SBI_DIMER_ALP_DETECTOR_5S_50FPS_Simulation_TRAIN \
...
SIM="${SIM:-$REPO/Script_Bank/HPC/SRM_AND_SBI_DIMER_ALP_DETECTOR_HPC_Simulation.sh}"
...
STATE="$HOME/detector_gen_controller_state.txt"
...
submit(){   # $1 = entry; echoes job id on stdout, logs to stderr
  ...
  # SRM_AND_SBI_DIMER_ALP_DETECTOR_<timing_label>_Simulation_<SPLIT>, with timing_label
...
log "===== DIMER DETECTOR production generation | CASES=$CASES | DRYRUN=$DRYRUN ====="
