#!/bin/bash
# =============================================================================
# Slurm HPC generation submitter (detector): DLI over the condition's trajectory tier,
# many tasks packed per node.
# =============================================================================
# The detector has NO RDS stage of its own. It re-images the CONDITION's RDS trajectory
# tier -- one tier per condition (the association setting is per condition), shared by both
# workflows, generated by the biology dispatcher's RDS-only simulation with the SAME CONDITION
# (`SRM_AND_SBI_MONOMER_DIMER_ALP_HPC_Submit.sh simulation SIM_STAGE=rds CONDITION=<FAB|INLB>`)
# -- with the six imaging parameters drawn from the detector prior (the training label) and
# the camera from the SCOPE box, under the labeling law of CONDITION. The tier must exist for
# every task this submission renders (same CONDITION, SPLIT, task ids, TASK_SIMS, and
# TOTAL_TIME); the DLI stage fails loud on a missing trajectory.
...
# CORE=100 production (TRAIN 8 / TEST 2 / EVAL 1; 1000 sims/task), one node each, over the
# existing FAB tier:
...
# The other condition is one more submission per split over ITS OWN tier (CONDITION=INLB re-images
# the INLB tier; job-name token _INLB_). Two nodes, 20 tasks (element 0 -> Task_0..9, element 1 -> Task_10..19):
...
# Grow TRAIN later (renders tasks 8..15 once the FAB tier holds them, re-rendering nothing):
...
CONDITION="${CONDITION:-}"
case "$CONDITION" in FAB|INLB) ;; *) echo "FATAL: CONDITION='$CONDITION' (use FAB|INLB; the detector simulation is a DLI pass over the condition's tier and always needs it)." >&2; exit 1;; esac

start=$(( TASK_OFFSET + ARRAY_ID * PER_NODE ))
end=$(( TASK_OFFSET + TASK_COUNT ))
echo "=== Detector Simulation (DLI over the condition's tier) | $(hostname) | split=${SPLIT} condition=${CONDITION} sims=${TASK_SIMS} time=${TOTAL_TIME}s | seed=None | tasks [${start}..$(( end - 1 ))] ==="
