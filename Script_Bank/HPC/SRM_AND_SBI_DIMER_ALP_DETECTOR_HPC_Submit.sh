...
# (SRM_AND_SBI_DIMER_ALP_DETECTOR_HPC_Generate_Controller.sh): DRYRUN=1 prints, DRYRUN=0
...
case "$STAGE" in
  ...
  evaluation)
    ...
    ;;
  experiment)
    SUBMIT_SCRIPT="$REPO/Script_Bank/HPC/SRM_AND_SBI_DIMER_ALP_DETECTOR_HPC_Experiment.sh"
    JOBNAME="SRM_AND_SBI_DIMER_ALP_DETECTOR_${timing_label}_Experiment"
