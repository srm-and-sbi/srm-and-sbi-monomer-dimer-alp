...
# SRM_AND_SBI_MONOMER_DIMER_ALP_<timing_label>_Evaluation; with no TOTAL_TIME set the
...
#   sbatch --job-name=SRM_AND_SBI_MONOMER_DIMER_ALP_FAB_2S_50FPS_Evaluation --export=ALL,REPO=$PWD,CONDITION=FAB,EVAL_TASKS=1 Script_Bank/HPC/SRM_AND_SBI_MONOMER_DIMER_ALP_HPC_Evaluation.sh
# Example (two nodes, EVAL set sharded across both -- add --nodes=N; --gres is per node):
#   sbatch --nodes=2 --gres=gpu:4 --job-name=SRM_AND_SBI_MONOMER_DIMER_ALP_FAB_2S_50FPS_Evaluation --export=ALL,REPO=$PWD,CONDITION=FAB,EVAL_TASKS=25 Script_Bank/HPC/SRM_AND_SBI_MONOMER_DIMER_ALP_HPC_Evaluation.sh
# -----------------------------------------------------------------------------
