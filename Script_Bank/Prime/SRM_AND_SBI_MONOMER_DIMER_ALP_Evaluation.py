...
workflows: ``srm_and_sbi_monomer_dimer_alp.evaluation_runner.run_evaluation`` (built on the shared
``evaluation`` numeric layer). This entry point is the biology shim; the detector twin
(``SRM_AND_SBI_MONOMER_DIMER_ALP_DETECTOR_Evaluation.py``) is identical except it builds the detector
