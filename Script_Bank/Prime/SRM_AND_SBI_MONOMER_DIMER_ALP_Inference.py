...
workflows: ``srm_and_sbi_monomer_dimer_alp.inference_runner.run_inference`` (which builds on the
shared ``inference_support`` training machinery). This entry point is the biology shim; the
detector twin (``SRM_AND_SBI_MONOMER_DIMER_ALP_DETECTOR_Inference.py``) is identical except it builds
...
Usage:
    MACHINE_PROFILE=<profile> python SRM_AND_SBI_MONOMER_DIMER_ALP_Inference.py \\
        --total-time-seconds 2.0 --epochs 5 --tasks 2

    # Resume training from the previously saved checkpoint:
    MACHINE_PROFILE=<profile> python SRM_AND_SBI_MONOMER_DIMER_ALP_Inference.py --resurrect
"""

import sys

from srm_and_sbi_monomer_dimer_alp.inference_runner import build_inference_parser, run_inference
