...
``srm_and_sbi_monomer_dimer_alp.inference_runner.run_inference`` (built on the shared ``inference_support``
training machinery). This entry point is the detector shim; the biology twin
(``SRM_AND_SBI_MONOMER_DIMER_ALP_Inference.py``) is identical except it builds the biology config (10
...
Usage:
    MACHINE_PROFILE=<profile> python SRM_AND_SBI_MONOMER_DIMER_ALP_DETECTOR_Inference.py \\
