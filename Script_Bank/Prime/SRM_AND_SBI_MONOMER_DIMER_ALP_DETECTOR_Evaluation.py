...
``srm_and_sbi_monomer_dimer_alp.evaluation_runner.run_evaluation`` (built on the shared ``evaluation``
numeric layer). This entry point is the detector shim; the biology twin
(``SRM_AND_SBI_MONOMER_DIMER_ALP_Evaluation.py``) is identical except it builds the biology config (10
...
Usage:
    MACHINE_PROFILE=<profile> python SRM_AND_SBI_MONOMER_DIMER_ALP_DETECTOR_Evaluation.py \\
        --total-time-seconds 2.0 --eval-tasks 2 --pool-mode unrestricted
"""

import sys

from srm_and_sbi_monomer_dimer_alp.evaluation_runner import build_evaluation_parser, run_evaluation
