...
Companion to `SRM_AND_SBI_MONOMER_DIMER_ALP_DETECTOR_Experiment_Temporal_Dynamics.py`, the detector shim of
...
→ **`SRM_AND_SBI_MONOMER_DIMER_ALP_Experiment_Temporal_Dynamics.md`**
...
where `<alias>` is `SRM_AND_SBI_MONOMER_DIMER_ALP_DETECTOR`, so the two workflows' results never collide.

```bash
MACHINE_PROFILE=<profile> python \
    Script_Bank/Analysis/SRM_AND_SBI_MONOMER_DIMER_ALP_DETECTOR_Experiment_Temporal_Dynamics.py \
