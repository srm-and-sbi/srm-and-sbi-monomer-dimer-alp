...
``srm_and_sbi_monomer_dimer_alp.posterior_calibration_runner.run_posterior_calibration`` (built on
the workflow-agnostic ``posterior_calibration`` kernel). This entry point is the detector
shim; the biology twin (``SRM_AND_SBI_MONOMER_DIMER_ALP_Posterior_Calibration.py``) is identical
...
Usage:
    MACHINE_PROFILE=<profile> python SRM_AND_SBI_MONOMER_DIMER_ALP_DETECTOR_Posterior_Calibration.py \\
        --total-time-seconds 2.0 --eval-tasks 10 --posterior-samples 1000
"""

import sys

from srm_and_sbi_monomer_dimer_alp.posterior_calibration_runner import (
