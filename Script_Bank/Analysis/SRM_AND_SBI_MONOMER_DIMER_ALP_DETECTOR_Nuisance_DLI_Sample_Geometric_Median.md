...
`SRM_AND_SBI_MONOMER_DIMER_ALP_DETECTOR_Nuisance_DLI.py` (`--build`): that is where the imaging artifact is minted
...
`SRM_AND_SBI_MONOMER_DIMER_ALP_DETECTOR`), in a `<alias>_<timing_label>_Nuisance_DLI_Sample_Geometric_Median/` directory:
...
`SRM_AND_SBI_MONOMER_DIMER_ALP_DETECTOR_Nuisance_DLI.md`. Experimental recordings: MET single-particle-tracking
