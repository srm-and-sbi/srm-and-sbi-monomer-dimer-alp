...
Usage (from the repo root):
    MACHINE_PROFILE=<profile> PYTHONPATH=$PWD python \
        Script_Bank/Analysis/SRM_AND_SBI_MONOMER_DIMER_ALP_Brightness_Stationarity_Audit.py
...
Outputs (analysis results are data and live in the Data_Bank, never the codebase):
    <data_bank_root>/Posit/SRM_AND_SBI_MONOMER_DIMER_ALP_Brightness_Stationarity_Audit/
        SRM_AND_SBI_MONOMER_DIMER_ALP_Brightness_Stationarity_Audit.md  (report)
...
import srm_and_sbi_monomer_dimer_alp.simulation_dli_support as dli  # noqa: E402

assert os.path.abspath(dli.__file__).startswith(REPO_ROOT), (
    "Imported srm_and_sbi_monomer_dimer_alp from outside this repo checkout: " + dli.__file__
...
OUT_DIR = os.path.join(str(dli.PARAMETERS.machine.data_bank_root), "Posit",
                       "SRM_AND_SBI_MONOMER_DIMER_ALP_Brightness_Stationarity_Audit")
REPORT = os.path.join(OUT_DIR, "SRM_AND_SBI_MONOMER_DIMER_ALP_Brightness_Stationarity_Audit.md")
