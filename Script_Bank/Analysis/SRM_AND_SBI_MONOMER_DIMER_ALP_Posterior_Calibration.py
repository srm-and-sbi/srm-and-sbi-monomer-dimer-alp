...
workflows: ``srm_and_sbi_monomer_dimer_alp.posterior_calibration_runner.run_posterior_calibration``
(built on the workflow-agnostic ``posterior_calibration`` kernel). This entry point is
the biology shim; the detector twin
(``SRM_AND_SBI_MONOMER_DIMER_ALP_DETECTOR_Posterior_Calibration.py``) is identical except it
...
Usage:
    MACHINE_PROFILE=<profile> python SRM_AND_SBI_MONOMER_DIMER_ALP_Posterior_Calibration.py \\
