...
``srm_and_sbi_monomer_dimer_alp.estimator_comparison_runner.run_estimator_comparison`` (built on the
workflow-agnostic ``estimator_comparison`` kernel). This entry point is the detector shim; the
biology twin (``SRM_AND_SBI_MONOMER_DIMER_ALP_Estimator_Comparison.py``) is identical except it builds
...
Usage:
    MACHINE_PROFILE=<profile> python SRM_AND_SBI_MONOMER_DIMER_ALP_DETECTOR_Estimator_Comparison.py \\
        --total-time-seconds 2.0 --a canonical --b -12.16
"""

import sys

from srm_and_sbi_monomer_dimer_alp.estimator_comparison_runner import (
