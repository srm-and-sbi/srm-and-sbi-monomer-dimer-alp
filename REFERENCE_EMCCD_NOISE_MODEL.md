...
**Camera parameterization.** The five camera parameters — `gamma` (`= g/C`, the **ADU-per-photoelectron**, the only gain quantity the videos identify), `kappa_o` (the optical background offset, incident photons — §5), `kappa_s` (read noise `σ`), `kappa_b` (the camera baseline `b`, the electronic bias added last), and `kappa_q` (the quantum efficiency) — are externally constrained rather than jointly inferred in this workflow and are **marginalized as the SCOPE camera nuisance** (`DETECTOR_WORKFLOW.md` §9.3): each is drawn per simulation from its a-priori box and rendered into every video. The EM gain `g` and conversion `C` are **not** resolved separately — only their ratio `γ` is identifiable (§9) — so they are held fixed at their nominal spec values (keys `kappa_g`, `kappa_c`), retained solely as metadata for the `γ`-vs-`g_spec/C_spec` drift check (§8). `QE` (key `kappa_q`) is applied once in the Poisson step (§2) — no longer held at 1 and absorbed into the emitter brightness scale — and the biological recordings pin only the product `γ·QE` (§9), which is why the camera is marginalized rather than fit jointly; controlled-illumination photon transfer (§8) constrains `γ` on its own. The physics symbols `g, C, σ, b` are used in this document for the math; the `kappa_`/`gamma` keys are the code identifiers. The MET-matched values (`g = 200`, `C = 4.78`, baseline `b ≈ 175`, `QE = 0.9`, pixel `158 nm`, hence `γ = g/C ≈ 41.84`) are those recorded in the public dataset's per-cell ThunderSTORM `cameraSettings` protocol (accession `S-BSST712`) — the same source the emitter-brightness and PSF values are drawn from (`DETECTOR_WORKFLOW.md` §6.7).
...
| Parameter | Symbol | Role | Prior (log₁₀) | Linear band | Center | Acquisition value (MET) | Evidence / source |
|---|---|---|---|---|---|---|---|
| `gamma` | `γ = g/C` | SCOPE nuisance | (1.62, 1.625) | 41.7 – 42.2 ADU/e⁻ | 41.9 | nominal `g/C = 200/4.78 ≈ 41.84` | acquisition setting / ThunderSTORM camera protocol (`S-BSST712`) |
| `kappa_o` | optical background offset | SCOPE nuisance | (1.455, 1.465) | 28.5 – 29.2 photons | 28.8 | offset median ≈ 28.7 (Fab 28.9 / InlB 28.6) | measured quantity / ThunderSTORM `offset[photon]` output column |
| `kappa_b` | camera baseline `b` | SCOPE nuisance | (2.24, 2.25) | 173.8 – 177.8 ADU | 175.8 | configured baseline ≈ 175 ADU | acquisition setting / ThunderSTORM camera protocol |
| `kappa_s` | read noise `σ` | SCOPE nuisance | (1.02, 1.025) | 10.5 – 10.6 ADU | 10.5 | read noise ≈ 50 e⁻ at 10 MHz ≈ 10.5 ADU at `C ≈ 4.78` | datasheet value / public camera specification (`S-BIAD1369`) |
| `kappa_q` | `QE` | SCOPE nuisance | (−0.05, −0.04) | 0.89 – 0.91 | 0.90 | configured QE = 0.9 (per-dataset) | acquisition setting / ThunderSTORM camera protocol; the recordings pin only `γ·QE` |
| `kappa_g` | `g` | fixed (metadata) | — | — | — | 200 (nominal, drift check) | acquisition setting / ThunderSTORM camera protocol |
| `kappa_c` | `C` | fixed (metadata) | — | — | — | 4.78 e⁻/ADU (nominal, drift check) | acquisition setting / ThunderSTORM camera protocol |
