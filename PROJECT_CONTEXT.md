...
that a parameter is recoverable. The regenerated detector Experiment products give
the empirical case for the rule (`DETECTOR_WORKFLOW.md` §9.8): one estimator's
bleaching MAP switches between two competing joint-density solutions while its
median and SGM stay at the prior center of a weakly constrained parameter, and its
skewed `sigma_r` density places the MAP a stable half-IQR below the median.
